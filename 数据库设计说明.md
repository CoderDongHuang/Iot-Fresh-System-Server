...
CREATE TABLE devices (
    ...
    status TINYINT DEFAULT 1 COMMENT '设备状态：1-在线，0-离线，2-故障，3-维护',
...
#### 状态值说明
- **1 (在线)**: 设备正常运行，能够接收和发送数据
- **0 (离线)**: 设备无法连接，超过心跳时间阈值
- **2 (故障)**: 设备出现硬件或软件故障
- **3 (维护)**: 设备处于维护状态，暂停服务
...
CREATE TABLE device_data (
    ...
    vstatus TINYINT COMMENT '设备运行状态：1-运行，0-停止',
...
- `device_id`: 关联的设备ID
- `vid`: 设备唯一标识
- `alarm_type`: 报警类型（temperature、humidity、light等）
- `alarm_level`: 报警级别（低、中、高、危急）
- `title`: 报警标题
- `description`: 报警详细描述
- `status`: 报警处理状态（pending、acknowledged、resolved、ignored）
...
#### 报警级别说明
- **low (低)**: 一般性警告，不影响系统运行
- **medium (中)**: 需要注意的问题，可能影响部分功能
- **high (高)**: 严重问题，需要立即处理
- **critical (危急)**: 系统关键问题，需要紧急处理
...
- `(alarm_type, status)` - 用于按报警类型和状态查询
...
### 已实现的表结构
1. **设备表 (devices)**: 完整的设备信息管理
2. **设备数据表 (device_data)**: 实时数据存储和历史查询
3. **报警表 (alarms)**: 报警记录管理和处理
4. **用户表 (users)**: 用户认证和权限管理

### 数据关系
- 设备与设备数据：一对多关系
- 设备与报警：一对多关系
- 用户与报警：多对多关系（通过处理者字段关联）
